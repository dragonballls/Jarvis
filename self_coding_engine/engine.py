...
class _WorkspaceTransaction:
    """Snapshot authorized files and roll them back on failed attempts."""
    ...
    def unexpected_changes(self) -> tuple[str, ...]:
        before = self._inventory()
        # The inventory is captured again after execution by compare(); this
        # method is intentionally separate so callers can choose when to scan.
        return before

    def _inventory(self) -> tuple[str, ...]:
        ignored = {".git", ".venv", "venv", "env", "__pycache__", "node_modules", "dist", "build", "coverage"}
        result: list[str] = []
        for path in self.workspace.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(self.workspace)
            if rel.parts and rel.parts[0].lower() in ignored:
                continue
...
class SelfCodingEngine:
    ...
    A host application supplies the actual planner, code executor, verifier,
    and optionally a repair callback. The engine owns workspace confinement,
    authorized-path enforcement, rollback, bounded retries, and event flow.
    """
    ...
    @staticmethod
    def _git_clean_subset(workspace: Path, changed: Iterable[str]) -> bool:
        if not (workspace / ".git").exists():
            return True
        wanted = {str(p).replace("\\", "/") for p in changed}
        ...
        for line in result.stdout.splitlines():
            if len(line) >= 4:
                actual.add(line[3:].replace("\\", "/"))
        return actual.issubset(wanted)
            ...
            try:
                for attempt_number in range(1, self.max_attempts + 1):
                    ...
                    try:
                        ...
                        unexpected = not self._git_clean_subset(root, transaction.expected)
                        changed = transaction.changed_paths()
                        if unexpected:
                            raise CodingEngineError("workspace contains unauthorized repository changes")
                        if not verified_ok:
                            raise CodingEngineError("coding verification failed")
                        success = True
                        attempt = CodingAttempt(attempt_number, True, changed)
                        attempts.append(attempt)
                        yield CodingEvent("attempt_completed", {"step": step.id, "attempt": attempt_number, "changed_paths": changed})
                        break
                    except Exception as exc:
                        error = str(exc)
                        transaction.rollback()
                        attempts.append(CodingAttempt(attempt_number, False, (), error))
                        yield CodingEvent("attempt_failed", {"step": step.id, "attempt": attempt_number, "error": error})
                        if attempt_number < self.max_attempts and self.repairer is not None:
                            previous_result = self.repairer(step, attempt_number + 1, previous_result or {"error": error}, root)
                        elif attempt_number >= self.max_attempts:
                            raise CodingEngineError(f"step {step.id} exhausted repair attempts: {error}") from exc
            finally:
                transaction.close()
...
def file_sha256(path: str | Path) -> str:
    """Small standalone helper useful to host applications and tests."""
